...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'ADD_TEMPLATE': {
      const tmpl = [action.template, ...state.savedTemplates];
      return { ...state, savedTemplates: tmpl };
    }
    case 'DELETE_TEMPLATE':
      return {
        ...state,
        savedTemplates: state.savedTemplates.filter((_, i) => i !== action.index),
      };
...
export function AppProvider({ children }) {
  ...
  const deleteTemplate = useCallback((index) => dispatch({ type: 'DELETE_TEMPLATE', index }), []);
